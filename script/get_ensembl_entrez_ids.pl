...
# PODNAME: get_ensembl_entrez_ids.pl
# ABSTRACT: Get Entrez IDs for all Ensembl genes
...
=head1 EXAMPLES
    ...
    perl \
        -Ibranch-ensembl-79/ensembl/modules \
        get_ensembl_entrez_ids.pl \
        | sort > data/danio_rerio_e79_entrez.txt
...
foreach my $slice ( @{$slices} ) {
    ...
    foreach my $gene ( @{$genes} ) {
        ...
        foreach my $link ( @{$links} ) {
            next if $link->dbname ne 'EntrezGene';
